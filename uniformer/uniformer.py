...
class MultiVideoUniformer(nn.Module):
    def __init__(
        self,
        *,
        num_classes,
        ...
        super().__init__()
        self.uniformer1 = Uniformer(
            num_classes=128,
            dims=dims,
            depths=depths,
            mhsa_types=mhsa_types,
    ...
    def forward(self, video1, video2, video3):
        feat1 = self.uniformer1(video1)  # [B, 128]
        feat2 = self.uniformer2(video2)
        feat3 = self.uniformer3(video3)

        # Concatenate features
        fused = torch.cat([feat1, feat2, feat3], dim=1)  # [B, 384]
